...
#' @param max_features Maximum number of features to visualize
#' @param ... Additional arguments to pass to the plot function [plotthis::BarPlot]
#' @return The plot
#' @export
#' @importFrom plotthis BarPlot
VizSeuratGeneQC <- function(object, max_features = 10, ...) {
    stopifnot("[VizSeuratQC] 'object' must be a Seurat object" = inherits(object, "Seurat"))
    ...
    features <- unique(object@misc$gene_qc$ncells$feature)
    if (length(features) > max_features) {
        features <- features[1:max_features]
    }
...
VizSeuratDoublets <- function(object, plot_type = c("dim", "pie", "pk", "pK"), palette = "Set2", ...) {
    ...
    if (plot_type == "dim") {
        CellDimPlot(object, group_by = col, ...)
