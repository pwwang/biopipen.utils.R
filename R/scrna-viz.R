...
VizDEGs <- function(
    ...
    object = NULL,
    plot_type = c(
        "volcano", "volcano_log2fc", "volcano_pct", "jitter", "jitter_log2fc",
    ...
    if (is.null(outprefix)) {
        cached <- Cache$new(
            list(
                ...
                cutoff, order_by, select, outprefix, other_args
            ),
...
#' If the specified layer is not found, and there are layers with the specified layer as prefix, it will join layers first and try again.
...
VizSeuratCellQC <- function(
    ...
    scatter_x = "nCount_RNA", palette = "Set1", assay = "RNA", layer = "counts",
    ...) {
    ...
    genes <- setdiff(unique(c(features, scatter_x)), colnames(object@meta.data))
    if (length(genes) > 0) {
        if (!assay %in% names(object@assays)) {
            stop(paste0("[VizSeuratQC] 'assay' not found in object: ", assay))
        }
        if (
            !layer %in% SeuratObject::Layers(object@assays[[assay]]) &&
            any(startsWith(SeuratObject::Layers(object@assays[[assay]]), paste0(layer, ".")))
        ) {
                warning(paste0("[VizSeuratQC] 'layer' not found in assay: ", layer, ". But found layers with the specified layer as prefix. Will join layers first."))
                object <- SeuratObject::JoinLayers(object, assay = assay)
                if (!layer %in% SeuratObject::Layers(object@assays[[assay]])) {
                    stop(paste0("[VizSeuratQC] 'layer' not found in assay after joining layers: ", layer))
                }
        }
    ...
    if (plot_type == "scatter") {
        ScatterPlot(
            df,
            x = scatter_x, y = "value", color_by = "QC",
        ...
        if (plot_type == "violin") {
            ViolinPlot(df,
                x = "Sample", y = "value", palette = palette, add_point = TRUE,
...
VizSeuratGeneQC <- function(
    ...
    if (plot_type == "histogram") {
        Histogram(object@misc$gene_qc,
            x = "Count", group_by = "QC", facet_by = "Sample",
            palette = palette, ylab = ylab, ...
        )
    } else if (plot_type == "box") {
        BoxPlot(object@misc$gene_qc,
            x = "Sample", y = "Count", group_by = "QC",
            palette = palette, ylab = ylab, ...
        )
    } else if (plot_type == "violin") {
        ViolinPlot(object@misc$gene_qc,
            x = "Sample", y = "Count", group_by = "QC",
            palette = palette, ylab = ylab, ...
        )
    } else if (plot_type == "ridge") {
...
#' )
#'
#' obj <- LoadSeuratAndPerformQC(meta,
#'     cache = FALSE, gene_qc = list(min_cells = 3),
...
VizSeuratMap2Ref <- function(
    ...
    for (feat in features) {
        ...
        if (endsWith(feat_q, ".score") && substr(feat_q, 1, nchar(feat_q) - 6) %in% colnames(query@meta.data) && feat_q == feat_r) {
            ...
            query_feat <- if (is.data.frame(query[[feat_q]])) {
                query[[feat_q]][, 1]
            } else {
                query[[feat_q]]
            }
            ref_feat <- if (is.data.frame(ref[[feat_r]])) {
                ref[[feat_r]][, 1]
            } else {
                ref[[feat_r]]
            }
            ...
            if (is.numeric(query_feat) != is.numeric(ref_feat)) {
                stop(paste0(
                    "[VizSeuratMap2Ref] 'query' and 'ref' must have the same type for feature: ", feat_q, ":", feat_r, ".\n",
    ...
    if (length(unique(feat_types)) > 1) {
        stop(
            "[VizSeuratMap2Ref] 'query' and 'ref' should be either 'numeric' or 'character/factor' for all features, \n",
